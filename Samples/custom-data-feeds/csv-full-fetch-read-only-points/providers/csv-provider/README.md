...
1.  Create a local configuration file the **providers/csv-provider/src** directory. Name it `csv-provider-config.json`. Copy the contents below into that file. The **url** field is URL where the **RunningShoeStores.csv** file is hosted. Here's how it should look, but your URL value may be different.
...
2.  In the **providers/csv-provider/cdconfig.json** file, add the following objects to the `service parameters` array so that it looks like this:
...
2.  In a web browser, navigate
    to\>http://localhost:8080/csv-provider/rest/services/FeatureServer/0/query,
